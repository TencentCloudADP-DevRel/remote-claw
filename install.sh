...
#   --gateway-url=URL  Portal 回连地址（如 ws://host:18789）
...
for arg in "$@"; do
    case "$arg" in
        ...
        --gateway-url=*)  PUBLIC_GATEWAY_URL="${arg#*=}" ;;
        --gateway-port=*) GATEWAY_PORT="${arg#*=}" ;;
        --image=*)        IMAGE="${arg#*=}" ;;
        --vnc-password=*) VNC_PASSWORD="${arg#*=}" ;;
        --help|-h)
            banner
            echo "用法: bash install.sh --portal-url=URL --token=TOKEN [--image=REF] [--public-ip=IP] [--gateway-url=URL]"
            exit 0 ;;
    esac
done
...
services:
  desktop:
    image: ${OPENCLAW_IMAGE:-tencentcloudadpdevrel/openclaw-desktop:latest}
